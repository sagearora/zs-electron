...
function SteriController({
    ...
    const start = async (user?: UserModel) => {
        if (!user) {
            showPin(() => start)
            return;
        }
    ...
    const finish = async (user?: UserModel) => {
        if (!user) {
            showPin(() => finish)
            return;
        }
    ...
    if (status === 'loading') {
        return <div className='bg-slate-100 p-4 rounded-xl shadow-lg mb-8'>
            <Button onClick={() => start(user)}>Start Cycle</Button>
        </div>
    }
    ...
    if (status === 'running') {
        return <div className='bg-slate-100 p-4 rounded-xl shadow-lg mb-8'>
            ...
            />
            <Button onClick={() => finish(user)}>Finish Cycle</Button>
        </div>
    }
